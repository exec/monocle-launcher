...
fn push_compacted_log_run(
    ...
    if count >= LOG_COMPACTION_THRESHOLD {
        ...
        let _ = write!(output, " (x{count} times - compacted by Monocle Launcher)");
...
async fn maybe_emit_log_compaction_warning(
    ...
    let _ = crate::event::emit::emit_warning(&format!(
        "Monocle Launcher has compacted {} repeated log lines in {} before displaying it for performance reasons.",
